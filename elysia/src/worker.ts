...
type FunctionType = {
  [type: string]: (data: number) => void;
};

const functions: FunctionType = {
  complexCalculation1(data: number) {
  ...
  complexCalculation2(data: number) {
  ...
  complexCalculation3(data: number) {
...
parentPort?.on("message", (msg) => {
  const { input, fn } = msg;
  const f = functions[fn] as (data: number) => void;
